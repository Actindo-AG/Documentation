...
- ![Toggle](../../Assets/Icons/Toggle.png "[Toggle]") *Apply from default*   
    Enable this toggle to apply the default value to the image names. The default value is displayed read-only in the field after activation. Disable the toggle to unlock the *Use ECM image name* toggle and the *Regular expression* field and to configure how the image names should be customized before uploading to the Magento 2 shop and to improve the SEO of your images. By default, the toggle is enabled.

- [Save]  
    Click this button to save any changes made.

- ![Toggle](../../Assets/Icons/Toggle.png "[Toggle]") *Use ECM image name*   
    Disable the toggle if you want the image names to be randomly generated by upload. Enable the toggle to upload the image names configured in the *ECM* module. By default, the toggle is enabled. This toggle is only unlocked when the *Apply from default* toggle is disabled.
