...
[!User Interface Workers](../UserInterface/02_Workers.md)
...
[comment]: <> (A worker is a... Workers define...)
...
4. Enter a name for the worker in the *Name* field. It is recommended to describe the execution mode and the retry algorithm in the name for better identification later.

5. Specify the retry algorithm in the *Retry algorithm* field, separated by commas. It defines the time intervals in seconds in which the worker execution should be retried after a webhook message could not be sent successfully. You have the following options: 
...
6. Define the number of jobs in case you have decided for parallel processing.  
...
[comment]: <> (Screenshots anonymisieren!)
...
[comment]: <> (Name Core1 Platform oder Actindo Core Platform? Einheitlichen durchg. prüfen.)
...
[comment]: <> (no new webhooks will be sent... -> no new webhooks message will be sent? Check! Ist webhooks nicht eher eine Methode, Information automatisch vom Server zum Client automatisch, ohne Anfrage, trigger-getrieben, zu schicken, also, webhook an sich ist nicht geschickt.)
...
[comment]: <> (Vgl. Workflows: disable and kill workers. Ähnlich?)
...
Click the [![SHUT DOWN ALL](../../Assets/Icons/StopCircle.png "[SHUT DOWN ALL]") SHUT DOWN ALL] button top right.   
...
4. Return to the *Workers* view.

5. Select the workers you want to delete.   
    The [![DELETE](../../Assets/Icons/Trash10.png "[DELETE]") DELETE] button is highlighted.

6. Click the [![DELETE](../../Assets/Icons/Trash10.png "[DELETE]") DELETE] button.   
