...
- *Data Type*   
  ...
  [comment]: <> (Vielleicht noch erwähnen, dass ein Data Type auswgewählt werden muss, damit die Drop-down-Liste Assigned sets aktiv wird? Oder bei Assigned sets?)
...
- *Assigned Sets*   
  ...
  > [Info] If you do not assign the attribute to an attribute set at this point, you will have to do it afterwards, see [Add an attribute to the set](../Integration/02_ManageAttributeSets.md#add-an-attribute-to-the-set).


**CONFIGURATION**

The *CONFIGURATION* section displays the additional configuration for the selected data type. The configuration fields differ depending on the selected data type. Some data types do not have further configuration settings and, therefore, do not include a *CONFIGURATION* section.  
...
[comment]: <> (Evtl. diese Info in beiden Toggles oben weglassen? Nicht relevant hier. -> Enable this toggle to create a multi-lingual attribute. Disable the toggle to create a single language attribute. Oder anders formulieren: If the toggle is enabled, the attribute is multi-language/multi-channel. If the toggle is disabled, the attribute is single language/channel.)
