# Operation

In this part of the *PIM* documentation, you will find the following chapters:

- [Manage a product](./01_ManageProducts.md)
  - [Create a product](./01_ManageProducts.md#create-a-product)
  - [Edit products](./01_ManageProducts.md#edit-products)
    - [Edit a single product](./01_ManageProducts.md#edit-a-single-product)
    - [Mass editing](./01_ManageProducts.md#mass-editing)
  - [Copy a product](./01_ManageProducts.md#copy-a-product)
  - [Create an offer in PIM](./01_ManageProducts.md#create-an-offer-in-PIM)
    - [Create a single connection offer in PIM](./01_ManageProducts.md#create-a-single-connection-offer-in-PIM)
    - [Create a multi-connection offer in PIM](./01_ManageProducts.md#create-a-multi-connection-offer-in-PIM)


- [Manage a variant](./02_ManageVariants.md)
  - [Create variants](./02_ManageVariants.md#create-variants)
    - [Define the variants basic data](./02_ManageVariants.md#define-the-variants-basic-data)
    - [Create a single variant](./02_ManageVariants.md#create-a-single-variant)
    - [Create multiple variants](./02_ManageVariants.md#create-multiple-variants)
  - [Add variants](./02_ManageVariants.md#add-variants)  
  - [Edit a variant](./02_ManageVariants.md#edit-a-variant)


- [Move a product](./03_MoveProducts.md)
  - [Archive a product](./03_MoveProducts.md#archive-a-product)
  - [Delete a product](./03_MoveProducts.md#delete-a-product)
  - [Permanently delete a product](./03_MoveProducts.md#permanently-delete-a-product)
  - [Restore a product](./03_MoveProducts.md#restore-a-product)


- [Manage the view](./04_ManageView.md)
  - [Select a language](./04_ManageView.md#select-a-language)
  - [Select a channel](./04_ManageView.md#select-a-channel)
  - [Select a product category](./04_ManageView.md#select-a-product-category)
  - [Select a variant view](./04_ManageView.md#select-a-variant-view)
  - [Create a view](./04_ManageView.md#create-a-view)
