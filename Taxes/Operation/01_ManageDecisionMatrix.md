...
The decision tax matrix is designed to help the system decide which tax zone, and therefore which tax rate, is applicable to an order. The decision matrix consists of a list of decision-making criteria based on the information provided by the *Invoicing* module.
...
5. Click the *Ship-to postcode* drop-down list and select the appropriate option. The following options are available:
  ...
  In case of doubt, there are many regular expressions testing tools available free of charge in the Internet, such as [Regex101](https://regex101.com/). For detailed information about this function, see [Define tax exceptions on ZIP code level](#define-tax-exceptions-on-ZIP-code-level).
...
4. Click the *Ship-to postcode* drop-down list to select a different option, if necessary, and/or the right field to modify the range or the regular expression. Use the keyboard to enter a different value.
...
As of July 2021, a new taxation system for cross-border business-to-customer transactions performed by online retailers within the UE applies. The previous tax delivery thresholds that were individually determined by each EU member state have been replaced by a single by a new EU-wide threshold of 10,000 EUR.
...
All these different tax scenarios can be configured using the decision matrix.
...
3. Click the *Country of origin* drop-down lists and select **Any** on the left and the country where the product is shipped from on the right.
...
5. Click the *Ship-to postcode* drop-down list and select the appropriate option. The following options are available:
  ...
  In case of doubt, there are many regular expressions testing tools available free of charge in the Internet, such as [Regex101](https://regex101.com/).
