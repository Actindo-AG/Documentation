...
It contains some dashlets with information on vCores, database storage utilization, ratio of errors and requests, and jobs currently running.
...
The engine room is opened. The current workspace is overlapped. It contains some dashlets with information on vCores, database usage, ratio of errors and requests, and jobs currently running.
...
## Clear tenant cache
Function to clear the cache of the current instance. This function removes all entries that are currently in the cache of the instance. It is used if there is a general issue in processing, and if changes are made directly through the database rather than through system logic. It is recommended to use this function only if it is really necessary. Clearing the cache will slow down the instance considerably. 
...
The cache is cleared. Afterwards, it will be rebuilt again. The processes on the system are significantly slowed down.
...
vCores (CPU core voltage) define the computing power of an instance for running parallel processes as well as database queries. Idle times such as calls waiting for response to external service providers are not included. 
...
Your company has purchased a certain number of vCores assigned the main account. If a sandbox is created, it is assigned a number of vCores from the main account. This reduces the number of vCores available in the main account. Note, the transfer of the vCores from the main account to a sandbox may take some time (about 5 minutes).   
...
The engine room allows you to monitor the required vCores of the current instance. With the vCore dashlet, you can monitor the needs of the last two hours, whereby the vCores are analyzed as average over several seconds. The dashlet displays the totals per every 30 seconds in a curve diagram. 
...
The number of vCores is displayed in a legend. Used vCores are shown in green, required vCores above the limit for a given time are shown in red.
...
The diagram has two y-axis. On the left side, the number of requests is displayed, on the right side the number of errors. In the x-axis, the hours from 0-7 o'clock are displayed.   
...
The number of requests is displayed per hour.
...
The number of errors is displayed per hour.
...
The currently running asynchronous jobs as well as the jobs waiting are displayed. The number of jobs is displayed as well as the percentage ratio of finished jobs. If no jobs are running, the text *No running jobs* is displayed.
