...
# Manage users
A user on the Core1 is a global entity as users can be invited to any account that exist on the Core1

Any available Core1 user can be invited to any account. The owner of the user will receive an email and actively decide wether he/she wants to join.
...
Assigning users to a user group is necessary to activate a user on a current instance. A user who is not assigned any group is not active in the instance. This means in reverse, you must remove a user from all groups on an instance to remove him or her from an account.
Groups are used to manage rights.
...
Users can only be edited in the main account they have been created.
Users can change their own personal information in any account [Edit profile settings](../UsingCore1/02_EditProfileSettings.md).
