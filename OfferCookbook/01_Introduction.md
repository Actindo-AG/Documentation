...
1. See [Define the workflow basic settings](../ActindoWorkFlow/Operation/01_ManageWorkflows.md#define-the-workflow-basic-settings) in the *Process Orchestration* documentation for a general description.

2. Create a workflow and define the following workflow basic settings:

| Field | Value | Comments | 
| ------- | ----- | ------- |
| *Select a name for your new workflow* | Create offer from complete PIM product | You can choose the name freely. It is recommended to use a descriptive and distinctive name. |
| *Select a unique key for your new workflow* | create_offer_from_pim_product | You can choose the key freely, as long as it is unique. No blank spaces are allowed. |
...
2. Create a trigger and configure the following settings depending on the trigger you want to use:
    ...
    1. *PIM product saved* trigger
        ...
        |*Model* | Actindo\Modules\Actindo\PIM\Models\PIMProduct | Path to data model, for example Actindo.Modules.Actindo.PIM.Models.PIMProduct. <br> For a list of data models, go to *Dev Tool > API > Data models*. |
...
[comment]: <> (add in comments column: process priority + unique check refer to whole process -> or better add it to triggers in Process Orchestration doc)
...
You can add several conditions to a trigger. Depending on the trigger *Condition fulfillment* setting, only one (**if any is met**) or all (**if all are met**) conditions will apply. 
...
| Field | Value | Comments | 
|---------|-------|----------|
|*Prefix* | changeset. | Select **changeset.** to refer to a property that has been changed. <br> Select **entity.** to refer to any property independently of its change status. <br> For detailed information, see [Add a condition](../ActindoWorkFlow/Operation/02_ManageTriggers.md#add-a-condition). |
...
| Process ID     | Unique numerical identifier assigned to a running process on the system |  Example?           |
