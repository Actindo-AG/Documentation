...
For example:
  ...
  - Entering **02**, or simply **2**, in front of the contra account means: correction key not set and tax key **2**

The BU key must be entered in front of the contra account, which must be filled to the maximum number of digits defined in the account settings. For example:
...
| 5 | Customized tax key | The cancellation key **5** allows to enter multi-digit tax keys. For this purpose, a selection window with the corresponding sub-keys is displayed after filling the *Contra account* field. |
...
| 7 | General reversal with key **5** - customized tax key | The cancellation key **7** is used to cancel postings that have been entered with the cancellation key **5**.|
...
[comment]: <> (Fehlende Info in Zeile 7 aus alter Doku genommen: https://www.actindo.com/de/doku/app-hilfe/handbuch/retailsuiteaccounting/latest/Grundlagen/Grundlagen.html Kurz getestet, scheint bei mir nicht zu funktionieren)
