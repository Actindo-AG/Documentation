# User interface

In this part of the *Accounting* documentation, you will find the following chapters:

- [Fiscal year](./00a_FiscalYear.md)
    - [Journals](./00a_FiscalYear.md#journals)


- [Post](./01_Book.md)


- [Postings header](./01_Header.md)
    - [Cash book takeover](./01_Header.md#cash-book-takeover)
    - [Bank data takeover](./01_Header.md#bank-data-takeover)
        - [Split posting](./01_Header.md#split-posting)
    - [Process](./01_Header.md#process)
    - [Delete](./01_Header.md#delete)
    - [Lock months](./01_Header.md#lock-months)
    - [Balance carried forward](./01_Header.md#balance-carried-forward)
    - [Foreign currency wizard](./01_Header.md#foreign-currency-wizard)
    - [Supplier receipts](./01_Header.md#supplier-receipts)
        - [Acquire a supplier receipt](./01_Header.md#acquire-a-supplier-receipt)
        - [Overview](./01_Header.md#overview)
        - [Financial accounting takeover](./01_Header.md#financial-accounting-takeover)
    - [Account balance](./01_Header.md#account-balance)
    - [Account sheet](./01_Header.md#account-sheet)
    - [Journal](./01_Header.md#journal)
    - [Balance list](./01_Header.md#balance-list)
    - [Open items](./01_Header.md#open-items)
    - [BWA](./01_Header.md#bwa)
    - [ProfitCenter](./01_Header.md#profitcenter)
    - [Cost object comparison](./01_Header.md#cost-object-comparison)
    - [Chart of accounts](./01_Header.md#chart-of-accounts)
    - [Debtors/creditors](./01_Header.md#debtorscreditors)


- [Postings input line](./01_InputLine.md)
    - [Set amount 1](./01_InputLine.md#set-amount-1)
    - [Set amount 2](./01_InputLine.md#set-amount-2)
    - [Split posting](./01_InputLine.md#split-posting)
    - [Attached document](./01_InputLine.md#attached-document)
    - [Attach document](./01_InputLine.md#attach-document)
        - [Upload file](./01_InputLine.md#upload-file)


- [POSTINGS](./01a_Bookings.md)
    - [Attached document](./01a_Bookings.md#attached-document)


- [BALANCES](./01b_Balances.md)


- [ACCOUNT SHEET](./01c_AccountSheet.md)
    - [Attached document](./01c_AccountSheet.md#attached-document)


- [OPEN ITEMS](./01d_OpenItems.md)  
    - [Attached document](./01d_OpenItems.md#attached-document)


- [SEARCH](./01e_Search.md)  
    - [Accounts](./01e_Search.md#accounts)
    - [Search](./01e_Search.md#search)


- [POST RECEIPTS](./01f_BookReceipts.md)  


- [Settings](./02_Settings.md)

- [DEBTORS/CREDITORS](./02a_DebtorsCreditors.md)

    - [Customer/supplier list](./02a_DebtorsCreditors.md#customersupplier-list)
        - [Customer/supplier context menu](./02a_DebtorsCreditors.md#customersupplier-context-menu)

    - [Create customer/supplier](./02a_DebtorsCreditors.md#create-customersupplier)
        - [Address/contact](./02a_DebtorsCreditors.md#addresscontact)
        - [Delivery addresses](./02a_DebtorsCreditors.md#delivery-addresses)
        - [Invoicing](./02a_DebtorsCreditors.md#invoicing)
        - [Additional fields](./02a_DebtorsCreditors.md#additional-fields)
        - [Drop shipping](./02a_DebtorsCreditors.md#drop-shipping)
        - [CRM information](./02a_DebtorsCreditors.md#crm-information)
        - [Notes](./02a_DebtorsCreditors.md#notes)

    - [Customer/supplier "Number"](./02a_DebtorsCreditors.md#customersupplier-number)
        - [Address/contact](./02a_DebtorsCreditors.md#addresscontact-1)
        - [Further addresses](./02a_DebtorsCreditors.md#further-addresses)
        - [Invoicing](./02a_DebtorsCreditors.md#invoicing-1)
        - [Additional fields](./02a_DebtorsCreditors.md#additional-fields-1)
        - [Content](./02a_DebtorsCreditors.md#content)
        - [Stock history](./02a_DebtorsCreditors.md#stock-history)
        - [Statistics](./02a_DebtorsCreditors.md#statistics)
        - [Drop shipping](./02a_DebtorsCreditors.md#drop-shipping-1)
        - [CRM information](./02a_DebtorsCreditors.md#crm-information-1)
        - [Notes](./02a_DebtorsCreditors.md#notes-1)

    - [Loyalty cards](./02a_DebtorsCreditors.md#loyalty-cards)

- [ACCOUNTS](./02b_Accounts.md)

- [TAX KEYS](./02c_TaxKeys.md)

- [FISCAL YEARS](./02d_FiscalYears.md)
    - [Import postings](./02d_FiscalYears.md#import-bookings)
    - [Export postings](./02d_FiscalYears.md#export-bookings)

- [COST ACCOUNTING](./02e_CostAccounting.md)
    - [Cost center](./02e_CostAccounting.md#cost-center)
    - [Cost object](./02e_CostAccounting.md#cost-object)
    - [Print cost center/object](./02e_CostAccounting.md#print-cost-centerobject)
        - [Allocation table](./02e_CostAccounting.md#allocation-table)
        - [Select address](./02e_CostAccounting.md#select-address)
        - [ProfitCenter](./02e_CostAccounting.md#profitcenter)

- [FIXED POSTINGS](./02f_FixedBookings.md)
    - [Fixed posting batch name](./02f_FixedBookings.md#fixed-booking--batch)

- [FOLLOW-ON POSTINGS](./02g_FollowOnBookings.md)

- [BALANCE CARRIED FORWARD](./02h_BalanceCarriedForward.md)

- [BWA ASSIGNMENT](./02i_BWAAssignment.md)
    - [Copy from predefined system report](./02i_BWAAssignment.md#copy-from-predefined-system-report)
    - [Create report](./02i_BWAAssignment.md#create-report)
    - [Edit "Report ID"](./02i_BWAAssignment.md#edit-report-id)
    - [Create report line](./02i_BWAAssignment.md#create-report-line)
    - [Edit "Report line ID"](./02i_BWAAssignment.md#edit-report-line-id)

- [SAP ACCOUNT MAPPING](./02j_SAPAccountMapping.md)

- [SAP TAX KEY MAPPING](./02k_SAPTaxKeyMapping.md)

- [ADVANCED](./02l_Advanced.md)


- [Extras](./03_Extras.md)

- [IMPORT](./03a_Import.md)
    - [Compeso 7.X](./03a_Import.md#compeso-7x)
        - [Compeso 7.X: Step 1](./03a_Import.md#compeso-7X-step-1)
        - [Compeso 7.X: Step 2](./03a_Import.md#compeso-7x-step-2)
    - [CSV](./03a_Import.md#csv)
        - [CSV: Step 1](./03a_Import.md#csv-step-1)
        - [CSV: Step 2](./03a_Import.md#csv-step-2)
        - [CSV: Step 3](./03a_Import.md#csv-step-3)
        - [CSV: Step 4](./03a_Import.md#csv-step-4)
        - [CSV: Step 5](./03a_Import.md#csv-step-5)
    - [CSV LS-Motor](./03a_Import.md#csv-ls-motor)
        - [CSV LS-Motor: Step 1](./03a_Import.md#csv-ls-motor-step-1)
        - [CSV LS-Motor: Step 2](./03a_Import.md#csv-ls-motor-step-2)
    - [GDI](./03a_Import.md#gdi)
        - [GDI: Step 1](./03a_Import.md#gdi-step-1)
        - [GDI: Step 2](./03a_Import.md#gdi-step-2)
        - [GDI: Step 3](./03a_Import.md#gdi-step-3)
        - [GDI: Step 4](./03a_Import.md#gdi-step-4)
        - [GDI: Step 5](./03a_Import.md#gdi-step-5)
    - [openTRANS](./03a_Import.md#opentrans)
        - [openTRANS: Step 1](./03a_Import.md#opentrans-step-1)
        - [openTRANS: Step 2](./03a_Import.md#opentrans-step-2)
    - [Schilling Accounting](./03a_Import.md#schilling-accounting)
        - [Schilling Accounting: Step 1](./03a_Import.md#schilling-accounting-step-1)
        - [Schilling Accounting: Step 2](./03a_Import.md#schilling-accounting-step-2)
    - [Import/export definitions](./03a_Import.md#importexport-definitions)
    - [Select file](./03a_Import.md#select-file)

- [EXPORT](./03b_Export.md)
    - [BMD](./03b_Export.md#bmd)
        - [BMD: Step 1](./03b_Export.md#bmd-step-1)
        - [BMD: Step 2](./03b_Export.md#bmd-step-2)
        - [BMD: Step 3](./03b_Export.md#bmd-step-3)
        - [BMD tax mapping](./03b_Export.md#bmd-tax-mapping)
    - [CSV](./03b_Export.md#csv)
        - [CSV: Step 1](./03b_Export.md#csv-step-1)
        - [CSV: Step 2](./03b_Export.md#csv-step-2)
        - [CSV: Step 3](./03b_Export.md#csv-step-3)
    - [DATEV CSV](./03b_Export.md#datev-csv)
        - [DATEV CSV: Step 1](./03b_Export.md#datev-csv-step-1)
        - [DATEV CSV: Step 2](./03b_Export.md#datev-csv-step-2)
        - [DATEV CSV: Step 3](./03b_Export.md#datev-csv-step-3)
    - [GDI](./03b_Export.md#gdi)
        - [GDI: Step 1](./03b_Export.md#gdi-step-1)
        - [GDI: Step 2](./03b_Export.md#gdi-step-2)
        - [GDI: Step 3](./03b_Export.md#gdi-step-3)
    - [GDPdU export](./03b_Export.md#gdpdu-export)
        - [GDPdU export: Step 1](./03b_Export.md#gdpdu-export-step-1)
        - [GDPdU export: Step 2](./03b_Export.md#gdpdu-export-step-2)
        - [GDPdU export: Step 3](./03b_Export.md#gdpdu-export-step-3)
    - [Open items](./03b_Export.md#open-items)
        - [Open items: Step 1](./03b_Export.md#open-items-step-1)
        - [Open items: Step 2](./03b_Export.md#open-items-step-2)
        - [Open items: Step 3](./03b_Export.md#open-items-step-3)
    - [Balance list](./03b_Export.md#balance-list)
        - [Balance list: Step 1](./03b_Export.md#balance-list-step-1)
        - [Balance list: Step 2](./03b_Export.md#balance-list-step-2)
        - [Balance list: Step 3](./03b_Export.md#balance-list-step-3)
    - [SALDEN.DAT](./03b_Export.md#salden-dat)
        - [SALDEN.DAT: Step 1](./03b_Export.md#saldendat-step-1)
        - [SALDEN.DAT: Step 2](./03b_Export.md#saldendat-step-2)
        - [SALDEN.DAT: Step 3](./03b_Export.md#saldendat-step-3)
    - [SAP](./03b_Export.md#sap)
        - [SAP: Step 1](./03b_Export.md#sap-step-1)
        - [SAP: Step 2](./03b_Export.md#sap-step-2)
        - [SAP: Step 3](./03b_Export.md#sap-step-3)
    - [Varial](./03b_Export.md#varial)
        - [Varial: Step 1](./03b_Export.md#varial-step-1)
        - [Varial: Step 2](./03b_Export.md#varial-step-2)
        - [Varial: Step 3](./03b_Export.md#varial-step-3)
    - [Zahlwerk](./03b_Export.md#zahlwerk)
        - [Zahlwerk: Step 1](./03b_Export.md#zahlwerk-step-1)
        - [Zahlwerk: Step 2](./03b_Export.md#zahlwerk-step-2)
        - [Zahlwerk: Step 3](./03b_Export.md#zahlwerk-step-3)
    - [Import/export definitions](./03a_Import.md#importexport-definitions)

- [CSV format definition](./03c_CSVFormatDef.md)
    - [New import/export definition](./03c_CSVFormatDef.md#new-importexport-definition)
    - [Rename import/export definition](./03c_CSVFormatDef.md#rename-importexport-definition)
