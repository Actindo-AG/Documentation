...
1. Click the *Status* drop-down list and select **All** to display all processes.  
...
3. Check the list displayed for the desired process to be checked.  
...
After selecting a process in the *OVERVIEW* tab, a read-only diagram of the selected process including all places and actions is displayed. If the process is currently active, the process execution can be observed in real time.
...
Depending on the filter settings, the actions contained in the selected processed are displayed.

  > [Info] When selecting a process in the *OVERVIEW* tab, the *Actions* tab is usually selected.

  [comment]: <> (By default? Actions Reiter scheint standarmäßig ausgewählt zu sein, wenn man einen Prozess aus der Liste anklickt. Das heißt, erster Schritt etwas überflüssig. Actions displayed? S. folgendes Kommentar)
