# User Interface

- [Overview](01_Overview.md)


- [DASHBOARDS](01a_Overview.md)
  - [Add a new tab - Create new tab](01a_Overview.md#add-a-new-tabcreate-new-tab)
  - [Add a new tab - Import existing tab](01a_Overview.md#add-a-new-tabimport-existing-tab)
  - [Confirm tab deletion](01a_Overview.md#confirm-tab-deletion)
  - [Tab](01a_Overview.md#tab)
    - [Edit tab](01a_Overview.md#edit-tab)
      - [Dashlet picker](01a_Overview.md#dashlet-picker)


- [Settings](02_Settings.md)


- [ATTRIBUTES](02a_Settings.md)
  - [Create attribute](02a_Settings.md#create-attribute)
  - [Edit attribute](02a_Settings.md#edit-attribute)
    - [Edit attribute - Data](02a_Settings.md#edit-attributedata)
    - [Edit attribute - Attribute set assignments](02a_Settings.md#edit-attributeattribute-set-assignments)


- [ATTRIBUTE SETS](02b_Settings.md)
  - [Create attribute set](02b_Settings.md#create-attribute-set)
  - [Edit attribute set](02b_Settings.md#edit-attribute-set)
    - [Edit attribute for set](02b_Settings.md#edit-attribute-for-set)
    - [Add mapping](02b_Settings.md#add-mapping)


- [ATTRIBUTE GROUPS](02c_Settings.md)
  - [Add attributes to group](02c_Settings.md#add-attributes-to-group)
  - [Edit attribute groups](02c_Settings.md#edit-attribute-groups)
    - [Add element](02c_Settings.md#add-element)
    - [Edit group data](02c_Settings.md#edit-group-data)


- [ETL](02d_Settings.md)
  - [Create ETL attribute set mapping](02d_Settings.md#create-etl-attribute-set-mapping)
  - [Mapping](02d_Settings.md#mapping)
    - [Settings](02d_Settings.md#settings)
    - [Rerun all mappings](02d_Settings.md#rerun-all-mappings)
    - [Rerun single mapping](02d_Settings.md#rerun-single-mapping)


- [CHANNELS](02e_Settings.md)
  - [Create channel](02e_Settings.md#create-channel)
  - [Edit channel](02e_Settings.md#edit-channel)


- [LANGUAGES](02f_Settings.md)
  - [Create language](02f_Settings.md#create-language)
  - [Edit language](02f_Settings.md#edit-language)


- [UNITS AND DIMENSIONS](02g_Settings.md)
  - [Detail list of dimensions](02g_Settings.md#detail-list-of-dimensions)


- [ETL PROCESSES](02h_Settings.md)
  - [Create view](02h_Settings.md#create-view)
  - [Rename view](02h_Settings.md#rename-view)


- [DELETED ATTRIBUTES](02i_Settings.md)


- [HTML TEMPLATES](02j_Settings.md)
  - [Create HTML template](02j_Settings.md#create-html-template)
  - [Edit HTML template](02j_Settings.md#edit-html-template)


- [ETL extensions list](03_ETLExtensions.md)


- [Data type list](04_DataTypeList.md)
